...
namespace benchmark {
    ...
    namespace internal {
        ...
        namespace {
            ...
            BenchmarkReporter::Run CreateRunReport(const benchmark::internal::BenchmarkInstance &b, const internal::ThreadManager::Result &results,
                IterationCount memory_iterations, const MemoryManager::Result &memory_result, double seconds, int64_t repetition_index) {
                ...
                if (!report.error_occurred) {
                    ...
                    if (memory_iterations > 0) {
                        ...
                        report.allocs_per_iter = memory_iterations ? static_cast<double>(memory_result.num_allocs) / memory_iterations : 0;
            ...
            public:
                BenchmarkRunner(const benchmark::internal::BenchmarkInstance &b_, std::vector<BenchmarkReporter::Run> *complexity_reports_)
                                                                                                                    : b(b_), complexity_reports(*complexity_reports_),
                                                                                                                      ...
                                                                                                                      pool(b.threads - 1),
                                                                                                                      iters(has_explicit_iteration_count ? b.iterations
                                                                                                                                                         : 1) {
                    ...
                    for (int repetition_num = 0; repetition_num < repeats; repetition_num++) { DoOneRepetition(repetition_num); }
            ...
            private:
                ...
                IterationResults DoNIterations() {
                    ...
                    for (std::size_t ti = 0; ti < pool.size(); ++ti) { pool[ti] = std::thread(&RunInThread, &b, iters, static_cast<int>(ti + 1), manager.get()); }
                ...
                IterationCount PredictNumItersNeeded(const IterationResults &i) const {
                    ...
                    const IterationCount max_next_iters =
                        static_cast<IterationCount>(std::lround(std::max(multiplier * static_cast<double>(i.iters), static_cast<double>(i.iters) + 1.0)));
