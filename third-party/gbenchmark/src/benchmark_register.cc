    ...
    namespace internal {
        ...
        bool BenchmarkFamilies::FindBenchmarks(std::string spec, std::vector<BenchmarkInstance> *benchmarks, std::ostream *ErrStream) {
            ...
            for (std::unique_ptr<Benchmark> &family : families_) {
                ...
                const std::vector<int> *thread_counts = (family->thread_counts_.empty() ? &one_thread : &static_cast<const std::vector<int> &>(family->thread_counts_));
                const size_t family_size = family->args_.size() * thread_counts->size();
                // The benchmark will be run at least 'family_size' different inputs.
                // If 'family_size' is very large warn the user.
                if (family_size > kMaxFamilySize) {
                    Err << "The number of inputs is very large. " << family->name_ << " will be repeated at least " << family_size << " times.\n";
                }
                ...
                for (auto const &args : family->args_) {
                    for (int num_threads : *thread_counts) {
                        ...
                        if (family->iterations_ != 0) {
                            instance.name.iterations = StrFormat("iterations:%lu", static_cast<unsigned long>(family->iterations_));
                        }
        ...
        Benchmark::Benchmark(const char *name)
                                                                                                            : name_(name), aggregation_report_mode_(ARM_Unspecified),
                                                                                                              time_unit_(kNanosecond),
                                                                                                              range_multiplier_(kRangeMultiplier), min_time_(0),
                                                                                                              iterations_(0), repetitions_(0),
                                                                                                              measure_process_cpu_time_(false), use_real_time_(false),
        ...
        Benchmark *Benchmark::Range(int64_t start, int64_t limit) {
            ...
            for (int64_t i : arglist) { args_.push_back({ i }); }
            return this;
        }
        ...
        Benchmark *Benchmark::Ranges(const std::vector<std::pair<int64_t, int64_t>> &ranges) {
            ...
            for (std::size_t i = 0; i < ranges.size(); i++) { AddRange(&arglists[i], ranges[i].first, ranges[i].second, range_multiplier_); }
        ...
        Benchmark *Benchmark::ArgsProduct(const std::vector<std::vector<int64_t>> &arglists) {
            CHECK(ArgsCnt() == -1 || ArgsCnt() == static_cast<int>(arglists.size()));

            std::vector<std::size_t> indices(arglists.size());
            const std::size_t total =
                std::accumulate(std::begin(arglists), std::end(arglists), std::size_t{ 1 }, [](const std::size_t res, const std::vector<int64_t> &arglist) {
                    return res * arglist.size();
                });
            std::vector<int64_t> args;
            args.reserve(arglists.size());
            for (std::size_t i = 0; i < total; i++) {
                for (std::size_t arg = 0; arg < arglists.size(); arg++) { args.push_back(arglists[arg][indices[arg]]); }
                args_.push_back(args);
                args.clear();

                std::size_t arg = 0;
                do { indices[arg] = (indices[arg] + 1) % arglists[arg].size(); } while (indices[arg++] == 0 && arg < arglists.size());
            }
        ...
        Benchmark *Benchmark::DenseRange(int64_t start, int64_t limit, int step) {
            ...
            for (int64_t arg = start; arg <= limit; arg += step) { args_.push_back({ arg }); }
            return this;
        }
        ...
        Benchmark *Benchmark::DisplayAggregatesOnly(bool value) {
            ...
            if (value) {
                aggregation_report_mode_ = static_cast<AggregationReportMode>(aggregation_report_mode_ | ARM_DisplayReportAggregatesOnly);
            } else {
                aggregation_report_mode_ = static_cast<AggregationReportMode>(aggregation_report_mode_ & ~ARM_DisplayReportAggregatesOnly);
            }
        ...
        Benchmark *Benchmark::DenseThreadRange(int min_threads, int max_threads, int stride) {
            ...
            for (auto i = min_threads; i < max_threads; i += stride) { thread_counts_.push_back(i); }
