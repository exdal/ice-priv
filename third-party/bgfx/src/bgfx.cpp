...
#define BGFX_CHECK_RENDER_THREAD() \
	BX_ASSERT((NULL != s_ctx && s_ctx->m_singleThreaded) || ~BGFX_API_THREAD_MAGIC == s_threadIndex, "Must be called from render thread.")
#else
#define BGFX_CHECK_API_THREAD()
#define BGFX_CHECK_RENDER_THREAD()
...
#define BGFX_CHECK_CAPS(_caps, _msg) \
	BX_ASSERT(0 != (g_caps.supported & (_caps)), _msg " Use bgfx::getCaps to check " #_caps " backend renderer capabilities.");
...
#define BGFX_CONFIG_MEMORY_TRACKING (BGFX_CONFIG_DEBUG && BX_CONFIG_SUPPORTS_THREADING)
	...
	void setPlatformData(const PlatformData &_data)
	{
		if (NULL != s_ctx)
		{
			BGFX_FATAL(true && g_platformData.ndt == _data.ndt && g_platformData.context == _data.context, Fatal::UnableToInitialize, "Only backbuffer pointer and native window handle can be changed after initialization!");
		}
...
#include "fs_clear7.bin.h"
	...
	void dump(const VertexLayout &_layout)
	{
		...
		{
			BX_TRACE("VertexLayout %08x (%08x), stride %d", _layout.m_hash, bx::hash<bx::HashMurmur2A>(_layout.m_attributes), _layout.m_stride);
			...
			for (uint32_t attr = 0; attr < Attrib::Count; ++attr)
			{
				if (UINT16_MAX != _layout.m_attributes[attr])
				{
					...
					BX_TRACE("\tattr %2d: %-20s num %d, type %d, norm [%c], asint [%c], offset %2d", attr, getAttribName(Attrib::Enum(attr)), num, type, normalized ? 'x' : ' ', asInt ? 'x' : ' ', _layout.m_offset[attr]);
				}
			}
		}
	}
...
#include "charset.h"
	...
	void TextVideoMemBlitter::init()
	{
		...
		m_texture = createTexture2D(width, height, false, 1, TextureFormat::R8, BGFX_SAMPLER_MIN_POINT | BGFX_SAMPLER_MAG_POINT | BGFX_SAMPLER_MIP_POINT | BGFX_SAMPLER_U_CLAMP | BGFX_SAMPLER_V_CLAMP, mem);
	...
	void EncoderImpl::submit(ViewId _id, ProgramHandle _program, OcclusionQueryHandle _occlusionQuery, uint32_t _depth, uint8_t _flags)
	{
		...
		{
			BX_ASSERT(m_occlusionQuerySet.end() == m_occlusionQuerySet.find(_occlusionQuery.idx), "OcclusionQuery %d was already used for this frame.", _occlusionQuery.idx);
	...
	{
		BX_WARN(m_frame->m_numBlitItems < BGFX_CONFIG_MAX_BLIT_ITEMS, "Exceed number of available blit items per frame. BGFX_CONFIG_MAX_BLIT_ITEMS is %d. Skipping blit.", BGFX_CONFIG_MAX_BLIT_ITEMS);
...
#undef CAPS_FLAGS
	...
	static void dumpCaps()
	{
		...
		BX_TRACE("");
		BX_TRACE("Capabilities (renderer %s, vendor 0x%04x, device 0x%04x):", s_ctx->m_renderCtx->getRendererName(), g_caps.vendorId, g_caps.deviceId);
...
#undef LIMITS
	...
	TextureFormat::Enum getViableTextureFormat(const bimg::ImageContainer &_imageContainer)
	{
		...
		{
			convert |= 0 == (formatCaps & BGFX_CAPS_FORMAT_TEXTURE_CUBE) && 0 != (formatCaps & BGFX_CAPS_FORMAT_TEXTURE_CUBE_EMULATED);
		}
		else if (_imageContainer.m_depth > 1)
		{
			convert |= 0 == (formatCaps & BGFX_CAPS_FORMAT_TEXTURE_3D) && 0 != (formatCaps & BGFX_CAPS_FORMAT_TEXTURE_3D_EMULATED);
		}
		else
		{
			convert |= 0 == (formatCaps & BGFX_CAPS_FORMAT_TEXTURE_2D) && 0 != (formatCaps & BGFX_CAPS_FORMAT_TEXTURE_2D_EMULATED);
		}
	...
	bool Context::init(const Init &_init)
	{
		...
		if (g_platformData.ndt == NULL && g_platformData.nwh == NULL && g_platformData.context == NULL && g_platformData.backBuffer == NULL && g_platformData.backBufferDS == NULL)
		{
		...
		g_caps.supported |= 0 | (BX_ENABLED(BGFX_CONFIG_MULTITHREADED) && !m_singleThreaded ? BGFX_CAPS_RENDERER_MULTITHREADED : 0) | (isGraphicsDebuggerPresent() ? BGFX_CAPS_GRAPHICS_DEBUGGER : 0);
	...
	void Context::shutdown()
	{
		...
		BX_ASSERT(
			m_layoutHandle.getNumHandles() == m_vertexLayoutRef.m_vertexLayoutMap.getNumElements(), "VertexLayoutRef mismatch, num handles %d, handles in hash map %d.", m_layoutHandle.getNumHandles(), m_vertexLayoutRef.m_vertexLayoutMap.getNumElements());
...
#undef CHECK_HANDLE_LEAK_NAME
	...
	void Context::swap()
	{
		...
		m_submit->m_textVideoMem->resize(
			m_render->m_textVideoMem->m_small, m_init.resolution.width, m_init.resolution.height);
...
#undef BGFX_RENDERER_CONTEXT
	...
	void Context::rendererExecCommands(CommandBuffer &_cmdbuf)
	{
		...
		if (NULL == m_renderCtx)
		{
			...
			default:
			{
				BX_ASSERT(CommandBuffer::RendererInit == command, "RendererInit must be the first command in command buffer before initialization. Unexpected command %d?", command);
				...
				{
					_cmdbuf.read(command);
					BX_ASSERT(CommandBuffer::End == command, "Unexpected command %d?", command);
					return;
				}
			}
			break;
		...
		do
		{
			...
			case CommandBuffer::UpdateTexture:
			{
				...
				uint32_t key = (handle.idx << 16) | (side << 8) | mip;
	...
	uint8_t getSupportedRenderers(uint8_t _max, RendererType::Enum *_enum)
	{
		...
		{
			if ((RendererType::Direct3D11 == ii || RendererType::Direct3D12 == ii) && windowsVersionIs(Condition::LessEqual, 0x0502))
			{
				continue;
			}
	...
	PlatformData::PlatformData()
		: ndt(NULL), nwh(NULL), context(NULL), backBuffer(NULL), backBufferDS(NULL)
	{
	}

	Resolution::Resolution()
		: format(TextureFormat::RGBA8), width(1280), height(720), reset(BGFX_RESET_NONE), numBackBuffers(2), maxFrameLatency(0)
	{
	}

	Init::Limits::Limits()
		: maxEncoders(BGFX_CONFIG_DEFAULT_MAX_ENCODERS), minResourceCbSize(BGFX_CONFIG_MIN_RESOURCE_COMMAND_BUFFER_SIZE), transientVbSize(BGFX_CONFIG_TRANSIENT_VERTEX_BUFFER_SIZE), transientIbSize(BGFX_CONFIG_TRANSIENT_INDEX_BUFFER_SIZE)
	{
	}

	Init::Init()
		: type(RendererType::Count), vendorId(BGFX_PCI_ID_NONE), deviceId(0), debug(BX_ENABLED(BGFX_CONFIG_DEBUG)), profile(BX_ENABLED(BGFX_CONFIG_DEBUG_ANNOTATION)), callback(NULL), allocator(NULL)
	{
	}
	...
	bool init(const Init &_userInit)
	{
		...
		if (true && !BX_ENABLED(BX_PLATFORM_EMSCRIPTEN || BX_PLATFORM_PS4) && RendererType::Noop != init.type && NULL == init.platformData.ndt && NULL == init.platformData.nwh && NULL == init.platformData.context && NULL == init.platformData.backBuffer && NULL == init.platformData.backBufferDS)
		{
...
#define BGFX_ENCODER(_func) reinterpret_cast<EncoderImpl *>(this)->_func
	...
	void Encoder::setVertexBuffer(
		uint8_t _stream, VertexBufferHandle _handle, uint32_t _startVertex, uint32_t _numVertices, VertexLayoutHandle _layoutHandle)
	{
	...
	void Encoder::setVertexBuffer(
		uint8_t _stream, DynamicVertexBufferHandle _handle, uint32_t _startVertex, uint32_t _numVertices, VertexLayoutHandle _layoutHandle)
	{
	...
	void Encoder::setVertexBuffer(
		uint8_t _stream, const TransientVertexBuffer *_tvb, uint32_t _startVertex, uint32_t _numVertices, VertexLayoutHandle _layoutHandle)
	{
	...
	{
		BX_ASSERT(false || !isValid(_occlusionQuery) || 0 != (g_caps.supported & BGFX_CAPS_OCCLUSION_QUERY), "Occlusion query is not supported! Use bgfx::getCaps to check BGFX_CAPS_OCCLUSION_QUERY backend renderer capabilities.");
	...
	void Encoder::setImage(uint8_t _stage, TextureHandle _handle, uint8_t _mip, Access::Enum _access, TextureFormat::Enum _format)
	{
		...
		_format = TextureFormat::Count == _format
					  ...
					  : _format;
		BX_ASSERT(_format != TextureFormat::BGRA8, "Can't use TextureFormat::BGRA8 with compute, use TextureFormat::RGBA8 instead.");
	...
	void Encoder::blit(ViewId _id, TextureHandle _dst, uint8_t _dstMip, uint16_t _dstX, uint16_t _dstY, uint16_t _dstZ, TextureHandle _src, uint8_t _srcMip, uint16_t _srcX, uint16_t _srcY, uint16_t _srcZ, uint16_t _width, uint16_t _height, uint16_t _depth)
	{
		...
		BX_ASSERT(src.m_format == dst.m_format, "Texture format must match (src %s, dst %s).", bimg::getName(bimg::TextureFormat::Enum(src.m_format)), bimg::getName(bimg::TextureFormat::Enum(dst.m_format)));
		BX_UNUSED(src, dst);
...
#undef BGFX_ENCODER
	...
	void allocTransientIndexBuffer(TransientIndexBuffer *_tib, uint32_t _num)
	{
		...
		BX_ASSERT(_num == _tib->size / 2, "Failed to allocate transient index buffer (requested %d, available %d). "
	...
	void allocTransientVertexBuffer(TransientVertexBuffer *_tvb, uint32_t _num, const VertexLayout &_layout)
	{
		...
		BX_ASSERT(_num == _tvb->size / _layout.m_stride, "Failed to allocate transient vertex buffer (requested %d, available %d). "
														 "Use bgfx::getAvailTransient* functions to ensure availability.",
	...
	bool allocTransientBuffers(bgfx::TransientVertexBuffer *_tvb, const bgfx::VertexLayout &_layout, uint32_t _numVertices, bgfx::TransientIndexBuffer *_tib, uint32_t _numIndices)
	{
		...
		if (_numVertices == getAvailTransientVertexBuffer(_numVertices, _layout) && _numIndices == getAvailTransientIndexBuffer(_numIndices))
		{
	...
	void allocInstanceDataBuffer(InstanceDataBuffer *_idb, uint32_t _num, uint16_t _stride)
	{
		...
		BX_ASSERT(_num == _idb->size / _stride, "Failed to allocate instance data buffer (requested %d, available %d). "
												"Use bgfx::getAvailTransient* functions to ensure availability.",
	...
	uint16_t getShaderUniforms(ShaderHandle _handle, UniformHandle *_uniforms, uint16_t _max)
	{
		...
		BX_WARN(0 == _max || num <= _max, "Shader has more uniforms that capacity of output array. Output is truncated (num %d, max %d).", num, _max);
	...
	static void isTextureValid(uint16_t _depth, bool _cubeMap, uint16_t _numLayers, TextureFormat::Enum _format, uint64_t _flags, bx::Error *_err)
	{
		...
		{
			_err->setError(BGFX_ERROR_TEXTURE_VALIDATION, "Texture3D is not supported! "
		...
		{
			_err->setError(BGFX_ERROR_TEXTURE_VALIDATION, "Texture array is not supported! "
		...
		{
			formatSupported = 0 != (g_caps.formats[_format] & (0 | BGFX_CAPS_FORMAT_TEXTURE_2D | BGFX_CAPS_FORMAT_TEXTURE_2D_EMULATED | BGFX_CAPS_FORMAT_TEXTURE_2D_SRGB));
		}
		...
		if (_cubeMap)
		{
			formatSupported = 0 != (g_caps.formats[_format] & (0 | BGFX_CAPS_FORMAT_TEXTURE_CUBE | BGFX_CAPS_FORMAT_TEXTURE_CUBE_EMULATED | BGFX_CAPS_FORMAT_TEXTURE_CUBE_SRGB));
			srgbCaps = BGFX_CAPS_FORMAT_TEXTURE_CUBE_SRGB;
		}
		else if (is3DTexture)
		{
			formatSupported = 0 != (g_caps.formats[_format] & (0 | BGFX_CAPS_FORMAT_TEXTURE_3D | BGFX_CAPS_FORMAT_TEXTURE_3D_EMULATED | BGFX_CAPS_FORMAT_TEXTURE_3D_SRGB));
		...
		if (!formatSupported)
		{
			_err->setError(BGFX_ERROR_TEXTURE_VALIDATION, "Texture format is not supported! "
		...
		if (0 != (_flags & BGFX_TEXTURE_SRGB) && 0 == (g_caps.formats[_format] & srgbCaps & (0 | BGFX_CAPS_FORMAT_TEXTURE_2D_SRGB | BGFX_CAPS_FORMAT_TEXTURE_3D_SRGB | BGFX_CAPS_FORMAT_TEXTURE_CUBE_SRGB)))
		{
	...
	static TextureHandle createTexture2D(BackbufferRatio::Enum _ratio, uint16_t _width, uint16_t _height, bool _hasMips, uint16_t _numLayers, TextureFormat::Enum _format, uint64_t _flags, const Memory *_mem)
	{
		...
		if (BX_ENABLED(BGFX_CONFIG_DEBUG) && NULL != _mem)
		{
			...
			BX_ASSERT(ti.storageSize == _mem->size, "createTexture2D: Texture storage size doesn't match passed memory size (storage size: %d, memory size: %d)", ti.storageSize, _mem->size);
		}
	...
	TextureHandle createTexture3D(uint16_t _width, uint16_t _height, uint16_t _depth, bool _hasMips, TextureFormat::Enum _format, uint64_t _flags, const Memory *_mem)
	{
		...
		if (BX_ENABLED(BGFX_CONFIG_DEBUG) && NULL != _mem)
		{
			...
			BX_ASSERT(ti.storageSize == _mem->size, "createTexture3D: Texture storage size doesn't match passed memory size (storage size: %d, memory size: %d)", ti.storageSize, _mem->size);
		}
	...
	TextureHandle createTextureCube(uint16_t _size, bool _hasMips, uint16_t _numLayers, TextureFormat::Enum _format, uint64_t _flags, const Memory *_mem)
	{
		...
		if (BX_ENABLED(BGFX_CONFIG_DEBUG) && NULL != _mem)
		{
			...
			BX_ASSERT(ti.storageSize == _mem->size, "createTextureCube: Texture storage size doesn't match passed memory size (storage size: %d, memory size: %d)", ti.storageSize, _mem->size);
		}
	...
	FrameBufferHandle createFrameBuffer(uint8_t _num, const Attachment *_attachment, bool _destroyTextures)
	{
		...
		BX_ASSERT(_num <= BGFX_CONFIG_MAX_FRAME_BUFFER_ATTACHMENTS, "Number of frame buffer attachments is larger than allowed %d (max: %d).", _num, BGFX_CONFIG_MAX_FRAME_BUFFER_ATTACHMENTS);
	...
	FrameBufferHandle createFrameBuffer(void *_nwh, uint16_t _width, uint16_t _height, TextureFormat::Enum _format, TextureFormat::Enum _depthFormat)
	{
		...
		BX_ASSERT(_format == TextureFormat::Count || bimg::isColor(bimg::TextureFormat::Enum(_format)), "Invalid texture format for color (%s).", bimg::getName(bimg::TextureFormat::Enum(_format)));
		BX_ASSERT(_depthFormat == TextureFormat::Count || bimg::isDepth(bimg::TextureFormat::Enum(_depthFormat)), "Invalid texture format for depth (%s).", bimg::getName(bimg::TextureFormat::Enum(_depthFormat)));
		return s_ctx->createFrameBuffer(
			_nwh, bx::max<uint16_t>(_width, 1), bx::max<uint16_t>(_height, 1), _format, _depthFormat);
	}
