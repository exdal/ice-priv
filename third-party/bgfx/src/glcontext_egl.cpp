...
#include "glimports.h"
		...
		void GlContext::create(uint32_t _width, uint32_t _height)
		{
			...
			if (NULL == g_platformData.context)
			{
				...
				DISPMANX_ELEMENT_HANDLE_T dispmanElement = vc_dispmanx_element_add(dispmanUpdate, dispmanDisplay, 0, &dstRect, 0, &srcRect, DISPMANX_PROTECTION_NONE, NULL, NULL, DISPMANX_NO_ROTATE);
				...
				for (uint32_t ii = 0; ii < 2; ++ii)
				{
					...
					if (hasEglKhrCreateContext)
					{
						...
						flags |= BGFX_CONFIG_DEBUG && hasEglKhrNoError ? 0 | EGL_CONTEXT_FLAG_NO_ERROR_BIT_KHR
																	   : 0;

						if (0 == ii)
						{
							flags |= BGFX_CONFIG_DEBUG ? 0 | EGL_CONTEXT_OPENGL_DEBUG_BIT_KHR
		...
		{
			return BX_ENABLED(0 | BX_PLATFORM_LINUX | BX_PLATFORM_WINDOWS | BX_PLATFORM_ANDROID)
