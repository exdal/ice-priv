...
Asset AssetFile::Get(const uint32_t pIndex)
{
	...
	const auto data = std::vector<uint8_t>(
		this->body.begin() + pos,
		this->body.begin() + pos + region.size);
...
int32_t AssetFile::GetIndex(const std::string &pName) const
{
	...
	{
		if (this->header.asset_register[i].name == pName)
			return i;
	}
...
void AssetFile::Append(const std::string &pName, const Asset &pAsset)
{
	...
	if (register_index == -1)
		return; // Full TODO: throw an error!

	auto pos = 0; // Figure out the correct position for the to write to.
	for (const auto &region : this->header.asset_register)
		pos += region.size;
...
{
	auto file = std::make_shared<AssetFile>(); // Our AssetFile is too big to be stored in our stack
	auto data = FileSystem::ReadBinaryFile(pPath);

	{
		memcpy(&file->header, &data[0], sizeof(AssetHeader));

		const auto size = -(data.begin() + sizeof(AssetHeader) - data.end());
		file->body.resize(size);
		memcpy(&file->body[0], &data[0] + sizeof(AssetHeader), size);
	}
...
int32_t AssetFile::GetEmptyEntry() const
{
	for (auto i = 0; i < asset_register_size; ++i)
	{
		if (this->header.asset_register[i].size <= 0)
			return i;
	}
