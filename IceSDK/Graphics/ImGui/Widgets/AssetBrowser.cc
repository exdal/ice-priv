...
void ShowToolTip(Assets::AssetFile *, Assets::AssetRegion *pEntry) {
    ...
    ImGui::BeginTooltip();
    {
        ...
        switch (pEntry->type) {
            ...
            case Assets::eAssetType::AnimatedTexture: typeString = "AnimatedTexture"; break;
...
void AssetBrowser::Init(Memory::Ptr<Assets::AssetManager> pManager) {
    g_CachedTexture.push_back(Graphics::Texture2D::Load("Unknown", std::vector<uint8_t>(&unknown_png[0], &unknown_png[unknown_png_len])));
    g_CachedTexture.push_back(Graphics::Texture2D::Load("Texture", std::vector<uint8_t>(&texture_png[0], &texture_png[texture_png_len])));
    g_CachedTexture.push_back(
        Graphics::Texture2D::Load("AnimatedTexture", std::vector<uint8_t>(&animated_texture_png[0], &animated_texture_png[animated_texture_png_len])));
    g_CachedTexture.push_back(Graphics::Texture2D::Load("Audio", std::vector<uint8_t>(&audio_png[0], &audio_png[audio_png_len])));
    g_CachedTexture.push_back(Graphics::Texture2D::Load("Video", std::vector<uint8_t>(&video_png[0], &video_png[video_png_len])));
    g_CachedTexture.push_back(Graphics::Texture2D::Load("Particle", std::vector<uint8_t>(&particle_png[0], &particle_png[particle_png_len])));
    // TODO:
    g_CachedTexture.push_back(Graphics::Texture2D::Load("Shader", std::vector<uint8_t>(&unknown_png[0], &unknown_png[unknown_png_len])));
    g_CachedTexture.push_back(Graphics::Texture2D::Load("Text", std::vector<uint8_t>(&unknown_png[0], &unknown_png[unknown_png_len])));
}
...
void AssetBrowser::Frame(Memory::Ptr<Assets::AssetManager> pManager) {
    ...
    {
        auto window_visible_x2 = ImGui::GetWindowPos()[0] + ImGui::GetContentRegionMax()[0];
        ...
        ImGui::BeginChild("AssetBrowser::FileView", ImVec2(-1, -1));
        {
            ...
            for (auto &assetFile : pManager->GetAll()) {
                for (auto &assetEntry : assetFile->header.asset_register) {
                    if (std::string(assetEntry.name).empty())
                        break;

                    auto typeTexture = g_CachedTexture.at((size_t)assetEntry.type);

                    auto last_button_x2 = ImGui::GetItemRectMax()[0];
                    auto next_button_x2 = last_button_x2 + ImGui::GetStyle().ItemSpacing[0] + 96.0f;

                    if (i < Assets::asset_register_size && next_button_x2 < window_visible_x2) {
                        ImGui::SameLine();
                    }

                    auto keySize = ImGui::CalcTextSize(assetEntry.name, nullptr, false, 0.0f);
                    ImGui::BeginChild((std::string(assetEntry.name) + "/content").c_str(), ImVec2(96.0f, 106.0f + keySize.y), false, ImGuiWindowFlags_AlwaysAutoResize);
                    {
                        ImGui::Image((ImTextureID)typeTexture->GetHandle().idx, ImVec2(96, 96));
