...
void ShaderManager::AppendShader(const std::string &pKey, const bgfx::RendererType::Enum pRenderer, const eShaderType pType, const std::vector<uint8_t> &pSrc) {
    ICESDK_PROFILE_FUNCTION();
    this->_shaders.push_back(Shader{ pKey, pRenderer, pType, pSrc });
}

bgfx::ShaderHandle ShaderManager::LoadShader(const std::string &pKey, const eShaderType pShaderType) {
    ICESDK_PROFILE_FUNCTION();
    Shader *p_shader = nullptr;

    for (auto &shader : this->_shaders) {
        if (shader.name == pKey && shader.type == pShaderType && shader.renderer == bgfx::getRendererType()) {
            p_shader = &shader;
            break;
        }
    }
    ...
    const auto *buffer = bgfx::copy(p_shader->code.data(), static_cast<uint32_t>(p_shader->code.size()));
...
bgfx::ProgramHandle ShaderManager::LoadProgram(const std::string &pKey) {
    ICESDK_PROFILE_FUNCTION();
    if (_programs.find(pKey) != _programs.end())
        return _programs[pKey];
