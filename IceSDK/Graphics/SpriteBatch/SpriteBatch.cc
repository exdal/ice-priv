...
SpriteBatch::SpriteBatch() {
    ...
    this->_indexHandle = bgfx::createIndexBuffer(bgfx::copy(quads, _maxIndices * sizeof(uint32_t)), BGFX_BUFFER_INDEX32);
    delete[] quads;
    ...
    for (uint32_t i = 0; i < _maxTextureSlots; i++) {
        ...
        this->_textureUniforms[i] = bgfx::createUniform(uni_name.c_str(), bgfx::UniformType::Sampler);
    }
...
void SpriteBatch::Flush() {
    if (!this->_indexes)
        return;
    uint32_t data_size = (uint32_t)((uint8_t *)this->_vertexBufferPtr - (uint8_t *)this->_vertexBuffer);
    ...
    bgfx::setIndexBuffer(this->_indexHandle, 0,
        count); // this is normal index buffer - bgfx::indexbufferhandle

    for (uint32_t i = 0; i < this->_textureIndex; i++) { bgfx::setTexture(i, this->_textureUniforms[i], this->_textureSlots[i]->GetHandle()); }
...
void SpriteBatch::SubmitTexturedQuad(Memory::Ptr<Texture2D> pTexture, const glm::vec2 &pPosition, const glm::vec2 &pSize, const glm::vec4 &pColour) {
    CheckIndexes();
...
void SpriteBatch::SubmitTiledSprite(
    Memory::Ptr<Texture2D> pTexture, const glm::vec2 &pPosition, const glm::vec2 &pSize, const glm::vec4 &pTileInfo, const glm::vec4 &pColour) {
    CheckIndexes();
    glm::mat4 transform = glm::translate(glm::mat4(1.0f), { pPosition.x, pPosition.y, 1 }) * glm::scale(glm::mat4(1.0f), { pSize.x, pSize.y, 1.0f });
...
void SpriteBatch::CheckIndexes() {
    if (this->_indexes >= _maxIndices)
        FlushReset();
}

void SpriteBatch::DrawIndexed(glm::mat4 pTransform, glm::vec4 pVertexPosition[QUAD_COUNT], std::array<glm::vec2, QUAD_COUNT> pUVs, const glm::vec4 &pColour,
...
float SpriteBatch::SetTexture(Memory::Ptr<Texture2D> pTexture) {
    ...
    if (textureIndex == 0.0f) {
        if (this->_textureIndex >= _maxTextureSlots)
            FlushReset();
...
std::array<glm::vec2, QUAD_COUNT> SpriteBatch::MakeTiled(Memory::Ptr<Texture2D> pTexture, const glm::vec4 &pTileInfo) {
