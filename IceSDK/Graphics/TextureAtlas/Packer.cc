...
Packer::Packer(uint32_t pWidth, uint32_t pHeight) : _width(pWidth), _height(pHeight), _usedSpace(0) {
    this->_skyline.push_back(Node(1, 1, this->_width));
}

bool Packer::Push(uint32_t pWidth, uint32_t pHeight, uint32_t &pXOut, uint32_t &pYOut) {
    pXOut = 0;
    pYOut = 0;
    ...
    for (uint32_t i = 0, size = this->_skyline.size(); i < size; i++) {
        ...
        if (y >= 0) {
            ...
            if (((uint32_t)y + pHeight) < (uint32_t)topH || (uint32_t)y + pHeight == (uint32_t)topH && node->width < (uint32_t)topW) {
...
int Packer::Fit(int index, uint32_t pWidth, uint32_t pHeight) {
    ...
    if ((x + pWidth) > (this->_width)) {
        return -1;
    }

    while (left > 0) {
        const Node &node = this->_skyline[idx];
        if (node.y > (uint32_t)y) {
            y = node.y;
        }

        if ((y + pHeight) > (this->_height)) {
            return -1;
        }
