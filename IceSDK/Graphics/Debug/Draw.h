...
namespace IceSDK::Graphics::Debug {
    ...
    inline void DrawQuad(glm::vec2 pPosition, glm::vec2 pSize, glm::vec4 pColour) {
        auto Model = glm::mat4{ 1 };
        auto View = glm::mat4{ 1 };
        auto Projection = glm::mat4{ 1 };

        const auto window = GetWindow();
        Projection = glm::ortho(0.0f, static_cast<float>(window->Width()), static_cast<float>(window->Height()), 0.0f, 0.1f, 1000.0f);
        Projection[3].z = 1;

        bgfx::setViewTransform(128, glm::value_ptr(View), glm::value_ptr(Projection));

        auto tex = Texture2D::Create("dbgBox", pSize.x, pSize.y, bgfx::TextureFormat::RGBA8);
        ...
        tex->Modify({ { 0.0, 0.0 }, pSize }, pixelData, bgfx::TextureFormat::RGBA8);

        auto vertexBuffer = bgfx::createVertexBuffer(bgfx::makeRef(g_SpriteVertices, sizeof g_SpriteVertices), g_2DPosTexCoordColourLayout);

        auto indexBuffer = bgfx::createIndexBuffer(bgfx::makeRef(g_SpriteIndices, sizeof g_SpriteIndices));

        const auto u_tex_colour = bgfx::createUniform("s_texColour", bgfx::UniformType::Sampler);
