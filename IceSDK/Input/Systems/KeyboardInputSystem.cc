...
void KeyboardInputSystem::Tick(float pDelta) {
    ...
    auto keyboardInputGroup = registry->view<Input::Components::KeyboardInputComponent>();
    for (auto rawKeyboardInputEntity : keyboardInputGroup) {
        auto keyboardEntity = Entity(this->_registry, rawKeyboardInputEntity);

        auto &keyboard = keyboardEntity.GetComponent<Input::Components::KeyboardInputComponent>();
