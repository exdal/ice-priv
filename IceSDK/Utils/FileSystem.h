...
namespace IceSDK {
    namespace FileSystem {
        ...
        constexpr bool HasExtension(std::string_view pPath, std::string_view pExt) noexcept {
        ...
        std::vector<std::string> ReadDirectory(std::string_view pPath, bool pRecursive = false);
        std::string ResolveFullPath(std::string_view pPath);
        uint8_t *ReadBinaryFile(std::string_view path, uint32_t *size);
        void WriteBinaryFile(std::string_view path, uint8_t *data, uint32_t size);
