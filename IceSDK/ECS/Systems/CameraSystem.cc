...
void CameraSystem::Draw(float pDelta)
{
	...
	bgfx::setViewTransform(0, glm::value_ptr(camera.view_matrix), glm::value_ptr(camera.projection_matrix));
}
...
void CameraSystem::Tick(float pDelta)
{
	...
	{
	case eCameraRenderingMode::Orthographic: // TODO: only execute if value changed
...
Entity CameraSystem::GetActiveCamera(const Memory::WeakPtr<entt::registry> pRegistry)
{
	...
	for (auto rawCameraEntity : cameraGroup)
	{
		...
		const auto &cam = cameraEntity.GetComponent<Components::CameraComponent>();
		if (cam.is_active)
		{
